...
| [ADR-015](./adr/ADR-015-ATTACHMENT-STORAGE-AND-QUOTAS.md) | Accepted | Use private S3-compatible storage engine, 25MB file/2GB account quotas, strict magic-byte MIME allowlist, authenticated download proxy, and async ClamAV virus scanning. Feature remains off until accepted. | Ensures strict privacy, data isolation, malware protection, and storage cost bounds while supporting dev/test parity with MinIO. |
