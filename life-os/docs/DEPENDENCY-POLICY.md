...
Dependabot checks the isolated LifeOS npm and Gradle directories every Monday in `Asia/Kolkata` and proposes changes against `develop`. Minor and patch updates are grouped per ecosystem; major updates remain separately visible for explicit compatibility and architecture review. Automation may open a proposal but cannot merge or deploy it.
