...
| LOS-0826 | Link tasks to milestones | Isolated `task_milestones` assignment table (one milestone per task; same-project + ownership invariants; clears on task/milestone delete) rather than a `Task` column, to avoid rippling the aggregate. API: assign/clear/get a task's milestone and list a milestone's tasks. UI: milestone picker in task detail/TaskForm, group tasks by milestone on Project Timeline/Overview. Lets users see which tasks belong to each phase/milestone instead of the label workaround. Backend + read UI (tasks grouped under milestones on the Timeline) complete; in-app assign control pending. | LOS-0704, LOS-0802, LOS-0813, LOS-0716 | M | In progress |
