...
/**
 * A fixed-window {@link SignupRateLimiter} delegating rate-limiting policy evaluation, metrics
...
class InMemorySignupRateLimiter implements SignupRateLimiter {
  ...
  @Autowired
  InMemorySignupRateLimiter(RateLimiterService rateLimiterService) {
  ...
  public boolean tryAcquire(String key) {
    return rateLimiterService.tryAcquire("signup:" + key, POLICY).allowed();
  }
}
