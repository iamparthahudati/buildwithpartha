...
/**
 * A fixed-window {@link PasswordResetRateLimiter} delegating rate-limiting policy evaluation,
...
class InMemoryPasswordResetRateLimiter implements PasswordResetRateLimiter {
  ...
  @Autowired
  InMemoryPasswordResetRateLimiter(RateLimiterService rateLimiterService) {
  ...
  public boolean tryAcquire(String key) {
    return rateLimiterService.tryAcquire("password-reset:" + key, POLICY).allowed();
  }
}
