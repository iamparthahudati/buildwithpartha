...
import tech.buildwithpartha.lifeos.report.application.TodayQueryResult.OverdueData;
...
/** Real-data provider for the Today overdue tasks widget (LOS-1415). */
@Component
public class DefaultOverdueWidgetProvider implements OverdueWidgetProvider {
  ...
  public DefaultOverdueWidgetProvider(
  ...
  public OverdueWidget getWidget(UUID userId, LocalDate localDate, ZoneId zoneId) {
    ...
    TodayOverdueRecord overdueRecord = todayTaskPort.getOverdueTasks(userId, now, zoneId, 5);

    if (overdueRecord.totalCount() == 0) {
    ...
    List<TodayTaskDto> taskDtos =
        overdueRecord.topOverdueTasks().stream()
            .map(
                task -> {
    ...
    return OverdueWidget.success(new OverdueData(overdueRecord.totalCount(), taskDtos));
  }
}
