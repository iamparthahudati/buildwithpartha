...
/**
 * A fixed-window {@link ResendVerificationRateLimiter} delegating rate-limiting policy evaluation,
 * metrics recording, and memory eviction to {@link RateLimiterService}.
 */
@Component
class InMemoryResendVerificationRateLimiter implements ResendVerificationRateLimiter {
  ...
  @Autowired
  InMemoryResendVerificationRateLimiter(RateLimiterService rateLimiterService) {
  ...
  public boolean tryAcquire(String key) {
    return rateLimiterService.tryAcquire("resend-verification:" + key, POLICY).allowed();
  }
}
