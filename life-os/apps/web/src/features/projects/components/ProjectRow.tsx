...
export function ProjectRow({
  ...
  const rootClass = ["lifeos-project-row", className].filter(Boolean).join(" ");
  ...
  if (loading || !project) {
    return (
      <Surface
        ...
        bordered
        padding="sm"
        className={[rootClass, "lifeos-project-row--loading"].join(" ")}
      >
        <VisuallyHidden>Loading project.</VisuallyHidden>
        <div className="lifeos-project-row__left">
          <Skeleton shape="circle" width="2.5rem" height="2.5rem" />
          <div className="lifeos-project-row__identity">
            <Skeleton shape="line" width="12rem" height="1.25rem" />
            <Skeleton shape="line" width="20rem" height="0.875rem" />
          </div>
        </div>
        <div className="lifeos-project-row__center">
          <Skeleton shape="line" width="6rem" height="1.5rem" />
          <Skeleton shape="line" width="8rem" height="0.75rem" />
        </div>
        <div className="lifeos-project-row__right">
          <Skeleton shape="line" width="5rem" height="1rem" />
  ...
  return (
    <Surface
      ...
      className={[
        rootClass,
        isArchived && "lifeos-project-row--archived",
        isOverdue && "lifeos-project-row--overdue",
      ]
        .filter(Boolean)
        .join(" ")}
    >
      <div className="lifeos-project-row__left">
        <div
          className="lifeos-project-row__icon-swatch"
          style={{ backgroundColor: projectColor }}
          aria-hidden="true"
        >
          <Icon icon={projectIcon} decorative size="md" className="lifeos-project-row__icon" />
        </div>
        <div className="lifeos-project-row__identity">
          <div className="lifeos-project-row__title-row">
            <Link
              href={projectHref}
              className="lifeos-project-row__name"
              aria-label={`Open project: ${project.name}`}
            >
              {project.name}
            </Link>
            {isArchived ? (
              <Badge tone="neutral" className="lifeos-project-row__archived-badge">
                Archived
              </Badge>
            ) : null}
            {isOverdue ? (
              <Badge tone="danger" className="lifeos-project-row__overdue-badge">
                Overdue
              </Badge>
            ) : null}
          </div>
          {project.description ? (
            <Text tone="secondary" size="xs" className="lifeos-project-row__description">
              {project.description}
            </Text>
          ) : null}
        </div>
      ...
      <div className="lifeos-project-row__center">
        <div className="lifeos-project-row__status-priority">
          ...
          <div className="lifeos-project-row__health">
        ...
        <div className="lifeos-project-row__progress-container">
          {progressTotal > 0 ? (
            <ProgressBar
              ...
              showValue
              size="sm"
              className="lifeos-project-row__progress"
            />
          ) : (
            <Text tone="muted" size="xs" className="lifeos-project-row__empty-progress">
              No tasks yet.
            </Text>
          )}
        </div>
      ...
      <div className="lifeos-project-row__right">
        <div className="lifeos-project-row__meta">
          <div className="lifeos-project-row__deadline">
          ...
          <div className="lifeos-project-row__updated">
        ...
        {menuItems.length > 0 ? (
          <div className="lifeos-project-row__actions">
            <Menu
              trigger={
                <IconButton
                  icon={MoreHorizontal}
